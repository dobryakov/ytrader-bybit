...
@router.post("/models/{version}/deactivate")
async def deactivate_model(version: str):
    """Proxy model deactivation request to model-service."""
    trace_id = get_or_create_trace_id()
    logger.info("model_deactivate_request", version=version, trace_id=trace_id)
    
    try:
        # Forward request to model-service
        model_service_url = f"http://{settings.model_service_host}:{settings.model_service_port}/api/v1/models/{version}/deactivate"
        ...
        logger.error("model_deactivate_failed", version=version, error=str(e), trace_id=trace_id, exc_info=True)
