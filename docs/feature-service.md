...
**Dataset Building:**
2. `POST /dataset/build` — собрать датасет за период с явным разделением на train/val/test.
   ...
   **Request body (time-based split):**
   ```json
   {
     ...
     "target_config": {
       "type": "return",  // или "direction", "sharpe_return"
       "horizon": "1m",    // 1m, 5m, 15m, 1h
       "threshold": 0.001  // для direction classification
     },
...
4. `GET /dataset/{dataset_id}` — получить метаданные датасета.
   - Возвращает: информацию о периодах, количестве записей, feature registry version, target config

5. `GET /dataset/{dataset_id}/download?split=train` — скачать датасет или его часть.
   - Query params: `split` = `train`, `validation`, `test`, или `all`
   - Возвращает: Parquet файл или ссылку на S3/объектное хранилище

**Model Evaluation:**
6. `POST /model/evaluate` — подготовить датасет для оценки модели на отдельном периоде.
   ```json
   {
     "dataset_id": "ds_20241201_001",
     "split": "test",  // или "validation"
     "model_version": "v123",  // опционально, для метаданных
...
**Feature Registry:**
7. `GET /feature-registry` — получить текущую версию Feature Registry.
8. `POST /feature-registry/reload` — перезагрузка конфигурации фичей.
9. `GET /feature-registry/validate` — валидация Feature Registry на data leakage.

**Data Quality:**
10. `GET /data-quality/report?symbol=BTCUSDT&from=...&to=...` — отчёт о качестве данных.
