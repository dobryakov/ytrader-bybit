...
В текущей реализации используется приватный эндпоинт Bybit (`/v5/private`), который поддерживает подписку как на приватные (wallet, position, order), так и на публичные (tickers, trade, orderbook, kline, liquidation) каналы.

В будущем планируется поддержка раздельных соединений:
- **Публичное соединение** (`/v5/public`) — для публичных данных (tickers, trade, orderbook, kline, liquidation)
- **Приватное соединение** (`/v5/private`) — для приватных данных (wallet, position, order)

Подробное описание архитектуры и плана реализации см. в [ws-gateway-public-endpoints.md](./ws-gateway-public-endpoints.md).
