...
Когда приходит торговый сигнал при наличии открытой позиции:
- **Любая позиция + любой сигнал** → позиция закрывается, затем создается новый ордер
- Это обеспечивает единообразное поведение: каждый новый сигнал закрывает старую позицию и открывает новую

### Процесс закрытия позиции

1. **Проверка наличия позиции**: Система проверяет, существует ли открытая позиция для данного актива
...
- **Тип**: `boolean`
- **По умолчанию**: `true`
- **Описание**: Включить автоматическое закрытие позиции перед обработкой любого сигнала. Если включено, при поступлении любого сигнала (независимо от направления) существующая позиция будет закрыта сначала, затем будет создан новый ордер
