...
**Model Service фокусируется исключительно на генерации торговых сигналов на основе рыночных данных:**

- **Генерация сигналов**: Использует ML-модель для анализа рыночных фич и генерации сигналов (buy/sell) с confidence score
- **Фичи от Feature Service**: Модель работает только с фичами, предоставленными Feature Service (те же фичи, на которых модель была обучена)
- **Без учета позиций**: Модель НЕ учитывает текущее состояние позиций при генерации сигналов
- **Без risk management**: Модель НЕ проверяет take profit/stop loss, лимиты размера позиций, не корректирует сумму ордера

**Order Manager отвечает за risk management и исполнение:**

- **Risk management**: Проверка take profit/stop loss, лимитов размера позиций, баланса
- **Адаптация ордеров**: Корректировка размера ордера с учетом текущей позиции и лимитов
- **Исполнение**: Создание, модификация и отмена ордеров на бирже

Такое разделение обеспечивает:
- **Чистоту модели**: Модель работает только с рыночными данными, на которых была обучена
- **Гибкость risk management**: Order Manager может применять различные стратегии управления рисками без изменения модели
- **Простота тестирования**: Модель и risk management тестируются независимо
