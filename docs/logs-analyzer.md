...
2. **Graylog Stack**
   ...
   - **Kibana** — дополнительная визуализация и аналитика логов (read-only доступ к индексам Graylog в Elasticsearch, один технический пользователь)
...
Docker GELF Driver (дублирование)
    ...
    └─→ Graylog (UDP 4711)
        ...
        Graylog Web UI (поиск и анализ)
            ↓
        Kibana Web UI (расширенная визуализация и аналитика, read-only)
```
...
1. **docker-compose.yml** ✅
   ...
   - ✅ Сервис `kibana` добавлен (read-only доступ к тем же индексам Elasticsearch, что использует Graylog)
...
2. **env.example** ✅
   ...
   - ✅ Добавлена переменная `KIBANA_PORT` для конфигурации внешнего порта Kibana
...
3. **README.md** (опционально)
   ...
   - Инструкции по доступу к Graylog Web UI и Kibana Web UI
...
| **kibana** | **4704** | **4704** | **HTTP** | **Веб-интерфейс (read-only аналитика по логам)** |
...
**Kibana Web UI (read-only доступ к индексам Graylog):**
```bash
http://localhost:4704
```
