...
class SignalPublisher:
    ...
    async def publish(self, signal: TradingSignal) -> None:
        ...
        # Persist trading signal to database FIRST (for both valid and rejected signals)
        # This ensures signal is available in database before it's published to RabbitMQ,
        # preventing race condition where execution events arrive before signal is persisted
        db_persisted = False
        try:
        ...
        if db_persisted and hasattr(signal, '_prediction_data'):
            try:
