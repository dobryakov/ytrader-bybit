...
- [X] T217 [P] [Enhancement] Verify and document class balancing implementation for XGBoost in model-service/src/services/model_trainer.py (for binary classification: verify that scale_pos_weight = negative_count / positive_count formula is correctly implemented, add structured logging to log scale_pos_weight value when calculated, document how scale_pos_weight helps with class imbalance and improves recall for minority classes; for multi-class classification: verify that sample_weight calculation is correct using formula class_weight = total_samples / (number_of_classes * class_count), verify that sample_weight array is properly created and passed to model.fit(), add structured logging to log class distribution and class weights for each class, verify that minority classes (e.g., "down" with 20.24% in current dataset) receive higher weights than majority class (e.g., "flat" with 48.78%), document in model-service/README.md how sample_weight helps with multi-class imbalance (current use case: 3 classes - flat, up, down), note: LightGBM is not used in this project - XGBoost was chosen as primary gradient boosting library per research.md)

- [X] T218 [P] [Enhancement] Implement SMOTE for class balancing in model-service/src/services/model_trainer.py (add optional SMOTE oversampling for minority classes before training, import imbalanced-learn library (imblearn.over_sampling.SMOTE), add configuration parameter MODEL_TRAINING_USE_SMOTE=false to enable/disable SMOTE (default: false to maintain backward compatibility), implement SMOTE application: apply only if MODEL_TRAINING_USE_SMOTE=true and task_type="classification" and number of classes > 1, handle edge cases: skip SMOTE if insufficient samples for minority classes (SMOTE requires at least k neighbors, default k=5), preserve class distribution information in logs before and after SMOTE, add structured logging: log original class distribution, SMOTE parameters (k_neighbors, sampling_strategy), new class distribution after SMOTE, number of synthetic samples generated, document in model-service/README.md when to use SMOTE vs sample_weight (SMOTE for severe imbalance, sample_weight for moderate imbalance), add imbalanced-learn>=0.11.0 to requirements.txt, note: SMOTE may increase training time and memory usage for large datasets, consider applying SMOTE only to train split, not validation/test splits)

- [X] T219 [P] [Enhancement] Optimize class weights calculation in model-service/src/services/model_trainer.py (improve sample_weight calculation to be more aggressive for severe class imbalance, add configuration parameter MODEL_TRAINING_CLASS_WEIGHT_METHOD="inverse_frequency" with options: "inverse_frequency" (current), "balanced" (sklearn.utils.class_weight.compute_class_weight), "custom" (from config), implement balanced class weights: use sklearn.utils.class_weight.compute_class_weight("balanced", classes=unique_labels, y=y) as alternative, add structured logging: log chosen weight method, original class distribution, computed class weights for each class, comparison of weights between methods, add configuration MODEL_TRAINING_CLASS_WEIGHT_METHOD="inverse_frequency" to settings.py with validation, document in model-service/README.md different weight methods and their use cases, add metrics: log weight statistics (min, max, mean weights) for monitoring)

- [X] T220 [P] [Enhancement] Implement hyperparameter optimization in model-service/src/services/model_trainer.py (add hyperparameter tuning using Grid Search or Bayesian Optimization, add configuration parameter MODEL_TRAINING_HYPERPARAMETER_TUNING=false to enable/disable tuning (default: false), add configuration MODEL_TRAINING_TUNING_METHOD="grid_search" with options: "grid_search", "bayesian" (using scikit-optimize or optuna), implement grid search: define parameter grid for XGBoost (max_depth: [3, 5, 7, 10], learning_rate: [0.01, 0.1, 0.3], n_estimators: [100, 200, 500], subsample: [0.8, 1.0], colsample_bytree: [0.8, 1.0], min_child_weight: [1, 3, 5]), use cross-validation with train/validation split for parameter evaluation, select best parameters based on validation set performance (F1-score for classification), add structured logging: log parameter search space, best parameters found, validation performance for best parameters, search duration, limit tuning time: add configuration MODEL_TRAINING_TUNING_MAX_ITERATIONS=50 or MODEL_TRAINING_TUNING_MAX_TIME_SECONDS=3600, add scikit-optimize>=0.9.0 or optuna>=3.0.0 to requirements.txt if using Bayesian optimization, document in model-service/README.md hyperparameter tuning process and recommendations, note: hyperparameter tuning significantly increases training time, use only for model improvement iterations, not for every training run)

- [X] T221 [P] [Enhancement] Add hyperparameter configuration options in model-service/src/config/settings.py (add MODEL_TRAINING_HYPERPARAMETER_TUNING: bool = Field(default=False, env="MODEL_TRAINING_HYPERPARAMETER_TUNING"), add MODEL_TRAINING_TUNING_METHOD: str = Field(default="grid_search", env="MODEL_TRAINING_TUNING_METHOD"), add MODEL_TRAINING_TUNING_MAX_ITERATIONS: int = Field(default=50, env="MODEL_TRAINING_TUNING_MAX_ITERATIONS"), add MODEL_TRAINING_USE_SMOTE: bool = Field(default=False, env="MODEL_TRAINING_USE_SMOTE"), add MODEL_TRAINING_CLASS_WEIGHT_METHOD: str = Field(default="inverse_frequency", env="MODEL_TRAINING_CLASS_WEIGHT_METHOD"), add validation for tuning_method in ["grid_search", "bayesian"], add validation for class_weight_method in ["inverse_frequency", "balanced", "custom"], update env.example with new configuration variables and descriptions)

- [X] T222 [P] [Enhancement] Add data quality validation in model-service/src/services/training_orchestrator.py (implement dataset quality checks before training: check for missing values in features, check for infinite values in features, check for constant features (zero variance), check for duplicate samples, check for data leakage (future information in features), add structured logging: log quality check results, number of missing values per feature, number of constant features, number of duplicate samples, add configuration MODEL_TRAINING_QUALITY_CHECKS_ENABLED=true to enable/disable quality checks (default: true), handle quality issues: log warnings for minor issues, raise errors for critical issues (e.g., data leakage), add metrics: track quality check failures and warnings, document in model-service/README.md data quality validation process and common issues)
...
### Task Dependencies for Walk-Forward Validation (T182-T188)

**Note**: These tasks implement walk-forward validation (rolling window) for training to increase data volume and improve model generalization through multiple overlapping folds.

- **T183** (walk-forward configuration): Can be done independently, must complete before T182 (training orchestrator needs configuration values)
- **T184** (env.example configuration): Can be done independently, can be done in parallel with T183
- **T182** (walk-forward support in orchestrator): Depends on T183 (configuration exists), T152 (training orchestrator exists), T145 (feature service client exists), must complete before T185 (fold selection needs walk-forward support)
- **T185** (fold selection logic): Depends on T182 (walk-forward support exists), T040 (model trainer exists), T041 (quality evaluator exists), must complete before T186 (regularization needs fold selection)
- **T186** (regularization for walk-forward): Depends on T185 (fold selection exists), T040 (model trainer exists), can be done in parallel with T187, T188
- **T187** (documentation update): Can be done independently, but should complete after T182-T186 (to document implemented walk-forward approach)
- **T188** (quickstart update): Can be done independently, but should complete after T182-T186 (to synchronize quickstart with walk-forward implementation)
- **Note**: T183 and T184 can be done in parallel (configuration tasks), T186, T187, and T188 can be done in parallel (enhancement and documentation tasks)
...
- **Total Tasks**: 183 tasks (added T052c, T052d, T052e for risk management rules, T094-T097 for position cache optimization, T098-T116 for position-based exit strategy, T123-T140 for training orchestrator improvements, T141 for balance adaptation investigation and fixes, T142-T144 for market data cache freshness and signal delay monitoring, T145-T163 for Feature Service integration, T173-T181 for test split evaluation and related updates, T182-T188 for walk-forward validation support, T217 for class balancing verification and documentation: scale_pos_weight for binary, sample_weight for multi-class, T189 for minimal feature set documentation)
...
- **Phase 7 (Polish)**: 42 tasks (added T091, T092, T093, T094-T097 for position cache optimization, T117-T122 for automatic asset selection, T141 for balance adaptation investigation and fixes, T142-T144 for market data cache freshness and signal delay monitoring, T217 for class balancing verification and documentation: scale_pos_weight for binary, sample_weight for multi-class, T218-T222 for model quality improvements: SMOTE for class balancing, optimized class weights, hyperparameter tuning, data quality validation)
...
- **Phase 12 (Walk-Forward Validation)**: 7 tasks (T182-T188 for implementing walk-forward validation support, adding configuration, implementing fold selection logic, adding regularization for overlapping data, updating documentation)
- **Phase 13 (Minimal Feature Set Documentation)**: 1 task (T189 for updating documentation with minimal feature set information, recommended horizon and threshold settings)
...
- [ ] T189 [P] [US2] [Enhancement] Add timestamp continuity validation using dataset metadata in model-service/src/services/training_orchestrator.py (extend dataset quality validation to check continuity_stats from dataset metadata: retrieve continuity_stats from dataset metadata when loading dataset from Feature Service, validate actual_coverage_ratio >= minimum threshold (configurable, default: 0.8 = 80%), check for critical gaps: if critical_gaps_count > 0 and largest_gap_seconds > threshold, log warning with gap details, check gap_count: if gap_count exceeds acceptable threshold, log warning with gap_locations (first 10 gaps), add structured logging: log continuity_stats when dataset is loaded, log warnings for low coverage ratio, log warnings for large gaps that may affect model training, add metrics: track datasets with continuity issues, track average coverage ratio across datasets, add configuration MODEL_TRAINING_MIN_DATASET_COVERAGE_RATIO=0.8 for minimum acceptable coverage ratio, add configuration MODEL_TRAINING_WARN_ON_LARGE_GAPS=true to enable/disable warnings for large gaps, integrate into _validate_data_quality method: include continuity_stats validation alongside existing quality checks, ensure backward compatibility: handle datasets without continuity_stats gracefully (old datasets from before continuity validation was implemented), document in model-service/README.md how continuity issues affect model training and recommendations for handling gaps)
- [ ] T190 [P] [US2] [Enhancement] Add configuration for timestamp continuity validation in model-service/src/config/settings.py (add MODEL_TRAINING_MIN_DATASET_COVERAGE_RATIO: float = Field(default=0.8, env="MODEL_TRAINING_MIN_DATASET_COVERAGE_RATIO", description="Minimum acceptable timestamp coverage ratio for training datasets. Datasets with coverage below this ratio will trigger warnings. Default: 0.8 (80% coverage). Lower coverage may indicate missing data periods that could affect model training quality."), add MODEL_TRAINING_WARN_ON_LARGE_GAPS: bool = Field(default=True, env="MODEL_TRAINING_WARN_ON_LARGE_GAPS", description="Enable warnings for large timestamp gaps in training datasets. Default: true. Large gaps may indicate missing trading sessions or data collection issues."), add MODEL_TRAINING_CRITICAL_GAP_THRESHOLD_SECONDS: int = Field(default=3600, env="MODEL_TRAINING_CRITICAL_GAP_THRESHOLD_SECONDS", description="Critical gap threshold in seconds. Gaps exceeding this duration will trigger warnings. Default: 3600 (1 hour). Gaps larger than this may significantly affect temporal dependencies in model training."), update env.example with new variables and documentation)
...
- [ ] T182 [US2] Add walk-forward validation support in model-service/src/services/training_orchestrator.py (add configuration check for MODEL_WALK_FORWARD_ENABLED, when enabled: use SplitStrategy.WALK_FORWARD instead of TIME_BASED in request_dataset_build(), construct WalkForwardConfig with train_window_days, validation_window_days, step_days, start_date, end_date from configuration, handle walk-forward dataset response which may contain multiple folds, add logic to process multiple folds: train model on each fold, evaluate on validation set, select best model based on validation metrics, use final test set for final evaluation, add structured logging for walk-forward operations including fold count, fold metrics, best fold selection)
- [ ] T183 [P] [US2] Add walk-forward configuration to model-service/src/config/settings.py (MODEL_WALK_FORWARD_ENABLED=false to enable/disable walk-forward validation, MODEL_WALK_FORWARD_TRAIN_WINDOW_DAYS=7 for training window size in days, MODEL_WALK_FORWARD_VALIDATION_WINDOW_DAYS=1 for validation window size in days, MODEL_WALK_FORWARD_STEP_DAYS=1 for step size between folds in days, MODEL_WALK_FORWARD_START_DATE_DAYS_AGO=30 for start date calculation (days ago from current time), MODEL_WALK_FORWARD_END_DATE_DAYS_AGO=1 for end date calculation (days ago from current time), document that walk-forward increases training data volume through overlapping windows, recommend regularization to prevent overfitting on overlapping data)
- [ ] T184 [US2] Add walk-forward configuration to env.example (MODEL_WALK_FORWARD_ENABLED=false, MODEL_WALK_FORWARD_TRAIN_WINDOW_DAYS=7, MODEL_WALK_FORWARD_VALIDATION_WINDOW_DAYS=1, MODEL_WALK_FORWARD_STEP_DAYS=1, MODEL_WALK_FORWARD_START_DATE_DAYS_AGO=30, MODEL_WALK_FORWARD_END_DATE_DAYS_AGO=1, document that step_days=1 creates maximum overlapping windows for more training data, step_days=7 creates less overlapping windows for more realistic validation)
- [ ] T185 [US2] Implement fold selection logic in model-service/src/services/training_orchestrator.py (when walk-forward generates multiple folds: train model on each fold's train set, evaluate on each fold's validation set, track validation metrics per fold, select best fold based on validation metrics (e.g., highest accuracy or ROC AUC), use selected fold's model for final test evaluation, log fold selection decision with metrics comparison, handle ties by selecting most recent fold)
- [ ] T186 [US2] Add regularization configuration for walk-forward training in model-service/src/services/model_trainer.py (when walk-forward enabled: increase L1/L2 regularization to prevent overfitting on overlapping data, add early stopping with patience to prevent memorization, add dropout if using neural networks, add configuration MODEL_WALK_FORWARD_REGULARIZATION_FACTOR=1.5 to multiply base regularization by factor, document that overlapping windows require stronger regularization)
- [ ] T187 [P] [US2] Update documentation in model-service/README.md (add section explaining walk-forward validation approach, document configuration parameters, explain benefits: more training data, better generalization, more realistic validation, explain risks: overfitting on overlapping data, increased computational cost, provide examples of walk-forward vs time-based split, recommend when to use each approach)
- [ ] T188 [P] [US2] Update quickstart.md in specs/001-model-service/ (add walk-forward validation examples, show configuration for rolling window approach, document fold selection process, align with feature-service walk-forward implementation)
...
- [ ] T189 [P] [Enhancement] Update documentation in model-service/README.md for minimal feature set in docs/feature-registry-todo.md (add section documenting minimal feature set (7 features): returns_5m, volatility_5m, rsi_14, ema_21, price_ema21_ratio, volume_ratio_20, funding_rate, document recommended prediction horizon: 5-15 minutes (300-900 seconds), document recommended classification threshold: 0.001-0.002 (0.1-0.2%), explain that minimal feature set is optimized for backfillable data sources (klines + funding rates), provide example target_config for dataset building with minimal features: {"type": "classification", "horizon": 300, "threshold": 0.001}, document that all features are computed from historical klines and funding rates available via REST API backfilling, reference docs/feature-registry-todo.md for complete feature set documentation)
...
- [X] T180 [P] Update quickstart.md in specs/001-model-service/ to reflect test set evaluation (synchronize training workflow with test set evaluation, update examples to show test set metrics, document that model activation uses test set metrics, align with T173 implementation and docs/feature-service.md workflow)
- [X] T181 [US2] Add warning logging for empty or unavailable test split in model-service/src/services/training_orchestrator.py (when test split download fails or test split is empty, log structured warning with dataset_id, symbol, reason (download_failed, empty_split, file_not_found), record warning in metrics for monitoring, document fallback behavior to validation metrics in logs, ensure warning is visible in monitoring dashboards for operational awareness)
