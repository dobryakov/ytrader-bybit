...
|-----------|---------|-----------|
| `trading_signals` | Trading signals generated by model-service | `id`, `signal_id`, `strategy_id`, `asset`, `side`, `price`, `confidence`, `timestamp`, `model_version`, `is_warmup`, `market_data_snapshot`, `metadata`, `trace_id`, `created_at` |
...
- Aggregate from multiple tables:
  - Trading signals: trading_signals table (timestamp for signal creation events)
...
**Source**: PostgreSQL `trading_signals` table (model-service persists all trading signals to database)
...
- `model_version` (VARCHAR) - Model version that generated the signal
...
```sql
-- Trading signal events (from trading_signals table)
SELECT 
