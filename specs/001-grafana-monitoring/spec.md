...
1. **Given** the system has generated trading signals, **When** a user accesses the Grafana dashboard, **Then** they see a list of recent trading signals (e.g., last 100 or last 24 hours) with all key attributes displayed
...
1. **Given** orders have been executed in the system, **When** a user accesses the Grafana dashboard, **Then** they see a list of recent orders (e.g., last 100 or last 24 hours) with execution status and closure information
...
- **FR-002**: System MUST display a list of recent trading signals (e.g., last 100 signals or last 24 hours) with key attributes: signal ID, asset, side, price, confidence, timestamp, strategy ID
...
- **Trading Signal**: Represents a trading decision generated by the model, including signal ID, asset, side (buy/sell), price, confidence score, timestamp, and strategy ID. Source: RabbitMQ queue `model-service.trading_signals` or database if persisted
...
- **SC-001**: Operators can view recent trading signals (last 100 or last 24 hours) within 3 seconds of accessing the dashboard
- **SC-002**: Operators can view recent orders with execution status (last 100 or last 24 hours) within 3 seconds of accessing the dashboard
