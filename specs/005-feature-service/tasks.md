...
- [ ] T402 [P] Fix and stabilize Feature Registry DB integration tests in feature-service/tests/integration/test_feature_registry_integration.py and feature-service/tests/integration/test_feature_registry_version_db.py (update mock_db_pool / MetadataStorage mocks to fully emulate asyncpg.Pool acquire/transaction semantics, remove reliance on real DB connections, ensure all version create/activate/load/rollback/startup fallback scenarios pass in isolation)
...
- [ ] T400 [US2] Add parity tests between online FeatureComputer and optimized offline pipeline (OptimizedDatasetBuilder/HybridFeatureComputer) in feature-service/tests/integration (replay a small slice of historical market data through online FeatureComputer, build an offline dataset for the same period and symbol with OptimizedDatasetBuilder, align by timestamp and symbol, compare feature vectors: ensure identical feature set (columns), compute max/mean absolute difference per feature, define acceptable tolerance per feature type, log/report discrepancies for further investigation)
- [ ] T401 [US2] Add end-to-end model inference compatibility test in feature-service/tests/integration (train or load a simple reference model on a dataset built by OptimizedDatasetBuilder, run the same raw market data through online FeatureComputer to produce online feature vectors, feed these online features into the model, verify that online predictions are numerically close to offline predictions for overlapping timestamps, ensure no schema/scale mismatches that could break inference in production)
- [ ] T402 [US2] Add post-build feature schema validation for OptimizedDatasetBuilder in feature-service/src/services/optimized_dataset/optimized_builder.py (after features_df is computed and before writing splits: load active FeatureRegistry for the dataset's feature_registry_version, build expected_features = {f.name for f in registry.features where f.is_required or default True}, build actual_features = set(features_df.columns) minus service columns like timestamp/symbol, compute missing_features = expected_features - actual_features and extra_features = actual_features - expected_features, if missing_features is not empty: log detailed error (dataset_id, feature_registry_version, missing_features, extra_features, available_columns), mark dataset as FAILED in MetadataStorage.update_dataset with appropriate error_message, do not write split files; optionally, support a per-feature flag in FeatureRegistry (e.g. required: bool) to allow non-fatal absence of experimental features)
