...
- Что происходит, когда ws-gateway недоступен или отправляет неполные данные?
- Как система обрабатывает рассинхронизацию orderbook (пропущенные delta-события)?
- Что происходит, когда данные о позициях недоступны (Position Manager недоступен)?
- Как система обрабатывает отсутствие исторических данных для запрошенного периода?
- Что происходит, если Feature Registry содержит конфигурацию с потенциальным data leakage?
- Как система обрабатывает запросы на признаки для символов, по которым нет данных?
- Что происходит при превышении минимального периода хранения данных (автоархивация/удаление)?
- Как система обрабатывает очень большой объем данных при сборке датасета?
- Что происходит, когда задержка вычисления признаков превышает допустимую?
- Как система обрабатывает одновременные запросы на сборку нескольких датасетов?
- Что происходит, когда очередь ws-gateway.order недоступна (опциональная очередь для execution events)?
- Как система обрабатывает восстановление orderbook из snapshot при обнаружении рассинхронизации?
- Что происходит, когда Feature Registry содержит невалидную конфигурацию (например, lookback_window больше max_lookback_days)?
